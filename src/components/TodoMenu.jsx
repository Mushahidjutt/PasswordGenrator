  ...
  return (
    <div>
      <div className=' flex justify-center items-center mt-4'>
        <input type="text" placeholder='Enter Task' className='w-[350px]' value={task} onChange={(e)=>setTask(e.target.value)} />
        <button  className='text-2xl' onClick={addTask}> <IoIosAddCircleOutline /> </button>
      </div>
        ...
        <div className={`${list.length>0 && 'bg-gradient-to-r from-sky-500 via bg-indigo-100 to-pink-100 pt-2 pl-2 pr-2'}`}>
          ...
          list.map((task)=>(
            <div className={`flex w-[350] text-md font-bold items-center space-x-2 bg-gray-200 p-2 rounded-md mb-2 
              ${task.toggle? 'bg-green-600 text-cyan-400' :'bg-violet-400'}`}
            key={task.id}>
